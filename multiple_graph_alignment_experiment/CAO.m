...
%     useCstInlier = target.config.useCstInlier;%set to 1 if use consistency to mask inliers, otherwise use affinity metrics
    ...
    constIterImmune = target.config.constIterImmune;% immune from consistency regularization, only use affinity in earl
    curIdx = 1;lastIdx = 2;
    constStep = target.config.constStep;% the inflate parameter, e.g. 1.05-1.1
