function [X, g_align_unrounded_score] = all_other_methods_compare_N_graphs(list_of_graphs, num_nodes,num_graphs)
...
%    *************************************************************************************
% sam % clear *;clear -global *;close all;clc;
global affinity target


%sam% varyMinGrhCnt=10;varyMaxGrhCnt=10;grhTestCnt = 50;%
...
nodeCnt = num_nodes;%sam% nodeCnt = target.config.nodeCnt;
graphCnt = num_graphs;%sam% graphCnt = target.config.graphCnt;
...
for testk = 1:testCnt
    %sam% affinity = generateRandomAffinity(nInlier,testk);
    ...
    for parak=1:paraCnt
        ...
        for grhOffset = 1:offsetStep:graphCnt-viewCnt+1%this logic can be ignored in the demo code, it is used for real image data
            ...
            if cao_Idx>0&&algSet.algEnable(cao_Idx)% use consistency-driven inlier elicination, by default for few oulier case
                ...
                %X{cao_Idx} = g_align_distance(affinity, nodeCnt, viewCnt, 0, 1);
